...
let _kbRequirements = [];
let _kbPendingDragId    = null;  
let _kbPendingPrevStage = null;  
...
function openKbScheduleModal(resumeId, candidate) {
    ...
    _kbPendingPrevStage = candidate._prevStage ?? null;  
...
async function kbSaveInterview() {
    ...
    const reqTitle   = document.getElementById("kbSReqTitle").value.trim();  // ← added
    ...
    if (reqTitle)  payload.position   = reqTitle;  // ← added
