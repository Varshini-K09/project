...
class ResumeSerializer(serializers.ModelSerializer):
    ...
    sharepoint_url     = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            "sharepoint_file", "sharepoint_url",
            "screening_result", "screening_done",
            "stage",
            "is_active",
            "created_at", "updated_at",
        ]
        read_only_fields = ["uploaded_by", "screening_result", "screening_done", "sharepoint_file"]

    def get_resume_url(self, obj):
        request = self.context.get("request")

        if obj.resume_file and obj.resume_file.name and request:
            return request.build_absolute_uri(obj.resume_file.url)

        if getattr(obj, "resume_pdf_bytes", None):
            if request:
                return request.build_absolute_uri(f"/api/resumes/{obj.pk}/file/")
            return f"/api/resumes/{obj.pk}/file/"

        return None

    def get_sharepoint_url(self, obj):
        if not obj.sharepoint_file:
            return None
        request = self.context.get("request")
        if request:
            return request.build_absolute_uri(f"/api/resumes/{obj.pk}/sharepoint-file/")
        return f"/api/resumes/{obj.pk}/sharepoint-file/"
